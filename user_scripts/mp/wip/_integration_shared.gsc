...
Setup()
{
    ...
    level.overrideMethods[level.commonFunctions.changeTeam]                        = user_scripts\mp\_integration_base::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.getTeamCounts]                     = user_scripts\mp\_integration_base::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.getTeamBased]                      = user_scripts\mp\_integration_base::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.getMaxClients]                     = user_scripts\mp\_integration_base::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.getClientTeam]                     = user_scripts\mp\_integration_base::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.getClientKillStreak]               = user_scripts\mp\_integration_base::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.backupRestoreClientKillStreakData] = user_scripts\mp\_integration_base::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.waitTillAnyTimeout]                = user_scripts\mp\_integration_base::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.getXuid]                           = user_scripts\mp\_integration_base::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.isBot]                             = user_scripts\mp\_integration_base::NotImplementedFunction;
    ...
    user_scripts\mp\_integration_base::_SetDvarIfUninitialized( level.commonKeys.autoBalance, 0 ); 
...
PlayerSpawnEvents() 
{
    ...
    self user_scripts\mp\_integration_base::RequestClientBasicData();
...
SaveTrackingMetrics()
{
    ...
    user_scripts\mp\_integration_base::LogDebug( "Saving tracking metrics for " + self.persistentClientId );
    ...
    user_scripts\mp\_integration_base::LogDebug( "Total Shots Fired increased by " + change );
    ...
    user_scripts\mp\_integration_base::IncrementClientMeta( "TotalShotsFired", change, self.persistentClientId );
}
...
OnBusModeRequestedCallback( event )
{
    ...
    user_scripts\mp\_integration_base::LogDebug( "Bus mode requested" );

    busModeRequest = user_scripts\mp\_integration_base::BuildEventRequest( false, level.eventTypes.getBusModeRequested, "", undefined, data );
    user_scripts\mp\_integration_base::QueueEvent( busModeRequest, level.eventTypes.getBusModeRequested, undefined );

    user_scripts\mp\_integration_base::LogDebug( "Bus mode updated" );
...
{
    user_scripts\mp\_integration_base::LogDebug( "Get commands requested" );
...
SendCommands( commandName )
{
    ...
    for ( i = 0; i < level.customCommands.size; i++ )
    {
        ...
        user_scripts\mp\_integration_base::LogDebug( "Sending custom command " + ( i + 1 ) + "/" + level.customCommands.size + ": " + data["name"] );
        commandRegisterRequest = user_scripts\mp\_integration_base::BuildEventRequest( false, level.eventTypes.registerCommandRequested, "", undefined, data );
        // not threading here as there might be a lot of commands to register
        user_scripts\mp\_integration_base::QueueEvent( commandRegisterRequest, level.eventTypes.registerCommandRequested, undefined );
    }
}
...
RegisterScriptCommand( eventKey, name, alias, description, minPermission, supportedGames, requiresTarget, handler )
{
    ...
    {
        user_scripts\mp\_integration_base::LogError( "eventKey must be provided for script command" );
        return;
    }
    ...
    {
        user_scripts\mp\_integration_base::LogError( "name must be provided for script command" );
        return;
    }
    ...
    {
        user_scripts\mp\_integration_base::LogWarning( "handler not defined for script command " + name );
    }
...
RequestUrl( url, method, body, headers, webNotify )
{
    ...
    user_scripts\mp\_integration_base::LogDebug( "next notify index is " + webNotify.index );
    ...
    webNotifyEvent = user_scripts\mp\_integration_base::BuildEventRequest( true, level.eventTypes.urlRequested, "", webNotify.index, data );
    thread user_scripts\mp\_integration_base::QueueEvent( webNotifyEvent, level.eventTypes.urlRequested, webNotify );
...
WaitForUrlRequestComplete()
{
    ...
    {
        user_scripts\mp\_integration_base::LogWarning( "Request to " + self.url  + " timed out" );
        self notify ( level.eventTypes.urlRequestCompleted, "error" );
    }

    user_scripts\mp\_integration_base::LogDebug( "Request to " + self.url  + " completed" );
...
OnUrlRequestCompletedCallback( event )
{
    ...
    {
        user_scripts\mp\_integration_base::LogWarning( "Incomplete data for url request callback. [1]" );
        return;
    }
    ...
    {
        user_scripts\mp\_integration_base::LogWarning( "Incomplete data for url request callback. [2] " + user_scripts\mp\_integration_base::CoerceUndefined( notifyEnt ) + " , " +  user_scripts\mp\_integration_base::CoerceUndefined( response ) );
        return;
    }
    ...
    {
        user_scripts\mp\_integration_base::LogDebug( "Additional data available for url request " + notifyEnt + " (" + event.data["remaining"] + " chunks remaining)" );
        return;
    }

    user_scripts\mp\_integration_base::LogDebug( "Notifying " + notifyEnt + " that url request completed"  );
...
OnPlayerDisconnect()
{
    ...
    for ( ;; )
    {
        ...
        user_scripts\mp\_integration_base::LogDebug( "client is disconnecting" );
...
OnPlayerJoinedTeam()
{
    ...
    for( ;; )
    {
        ...
        user_scripts\mp\_integration_base::LogDebug( self.name + " switched to " + newTeam );
        
        if ( newTeam != level.commonKeys.team1 && newTeam != level.commonKeys.team2 )
        {
            OnTeamSizeChanged();
            user_scripts\mp\_integration_base::LogDebug( "not force balancing " + self.name + " because they switched to spec" );
            continue;
        }
...
OnPlayerFirstSpawn()
{
    ...
    user_scripts\mp\_integration_base::LogDebug( "moving " + self.name + " to spectator because they did not spawn within expected duration" );
...
OnTeamSizeChanged()
{
    ...
    {
        user_scripts\mp\_integration_base::LogDebug( "not enough clients to autobalance" );
        return;
    }

    if ( !IsDefined( GetSmallerTeam( 1 ) ) )
    {
        user_scripts\mp\_integration_base::LogDebug( "teams are not unbalanced enough to auto balance" );
        return;
    }
    ...
    user_scripts\mp\_integration_base::LogDebug( "swapping " + toSwap.name + " from " + curentTeam + " to " + otherTeam );
...
FindClientToSwap()
{
    ...
    if ( !IsDefined( client ) )
    {
        user_scripts\mp\_integration_base::LogDebug( "could not find candidate to swap teams" );
    }
    else
    {
        user_scripts\mp\_integration_base::LogDebug( "best candidate to swap teams is " + client.name );
    }
...
GetBestSwapCandidate( team )
{
    ...
    for ( i = 0; i < maxClients; i++ )
    {
        ...
        user_scripts\mp\_integration_base::LogDebug( "candidate killstreak is " + candidateKillStreak );
        
        if ( candidateKillStreak > 3 )
        {
            user_scripts\mp\_integration_base::LogDebug( "skipping candidate selection for " + candidate.name + " because their kill streak is too high" );
            continue;
        }

        if ( IsDefined( candidate.autoBalanceCount ) && candidate.autoBalanceCount > 2 )
        {
            user_scripts\mp\_integration_base::LogDebug( "skipping candidate selection for " + candidate.name + " they have been swapped too many times" );
            continue;
        }
...
GetClosestPerformanceClientForTeam( sourceTeam, excluded )
{
    ...
    for ( i = 0; i < players.size; i++ )
    {
        ...
        user_scripts\mp\_integration_base::LogDebug( "perf=" + clientPerformance + " candidateValue=" + candidateValue + " src=" + sourceTeamNewPerformance + " dst=" + otherTeamNewPerformance );
        ...
        {
            user_scripts\mp\_integration_base::LogDebug( candidateValue + " is the new best value " );
            choice = players[i];
            closest = candidateValue;
        }
    }

    user_scripts\mp\_integration_base::LogDebug( choice.name + " is the best candidate to swap" + " with closest=" + closest );
    return choice;
}
...
GetTeamToJoin()
{
    ...
    {
        user_scripts\mp\_integration_base::LogDebug( "Team1 performance is lower, so selecting Team1" );
        return level.commonKeys.team1;
    }
    
    else
    {
        user_scripts\mp\_integration_base::LogDebug( "Team2 performance is lower, so selecting Team2" );
...
GetSmallerTeam( minDiff )
{
    ...
    {
        user_scripts\mp\_integration_base::LogDebug( "Team2 is full, so selecting Team1" );
        return level.commonKeys.team1;
    }
    
    if ( team1Count == maxClients / 2 )
    {
        user_scripts\mp\_integration_base::LogDebug( "Team1 is full, so selecting Team2" );
        return level.commonKeys.team2;
    }
    ...
    {
        user_scripts\mp\_integration_base::LogDebug( "Team size differs by more than 1" );
        
        if ( team1Count < team2Count )
        {
            user_scripts\mp\_integration_base::LogDebug( "Team1 is smaller, so selecting Team1" );
            return level.commonKeys.team1;
        }
        
        else
        {
            user_scripts\mp\_integration_base::LogDebug( "Team2 is smaller, so selecting Team2" );
            return level.commonKeys.team2;
        }
    }
